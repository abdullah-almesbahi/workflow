    ...
    private function getAliases()
    {
        if ($this->aliases === null) {
            ...
            foreach (Yii::$app->i18n->translations as $name => $translation) {
                ...
                $basePath = realpath(Yii::getAlias($translation->basePath));
        ...
        if (Yii::$app->request->isPost && !is_null(Yii::$app->request->post('messages'))) {
            ...
            if (!$hasErrors) {
                try {
                    ...
                    Yii::$app->session->setFlash('success', Yii::t('admin', 'Messages has been saved'));
                    $this->refresh();
                    Yii::$app->end();
                } catch (\Exception $e) {
                    Yii::$app->session->setFlash('error', Yii::t('admin', 'Cannot save messages'));
                }
            } else {
                Yii::$app->session->setFlash('error', Yii::t('admin', 'Wrong data'));
            }
        } else {
            try {
                ...
                $messages = '{}';
                Yii::$app->session->setFlash('error', Yii::t('admin', 'Cannot read messages'));
            }
        }
        return $this->render(
            'update',

...
use backend\widgets\image\SaveInfoAction;
...
class CrudController extends \common\controllers\CrudController
{
    ...
    {
        Yii::$app->session->setFlash('error', Yii::t('admin', 'Record deleted successfully'));
                            ...
                            function() use ( $i , $col , $user_value , $model ) {
                                ...
                                switch($c_action[$i]){
                                    ...
                                    case 'sms':
                                        ...
                                        $c_option = unserialize($col['c_option']);
                                        $smsModule = Sms::find()->where(['status' => 'activated'])->one();
                                        if (is_null($smsModule)) {
                                            throw new NotFoundHttpException;
                                        }
                                        $template = MailerTemplate::findOne($c_template[$i]);
                                        $sms = $smsModule->sms;
                                        $sms->msg = $template->body;
                                        $sms->to = explode(',',$c_option[$i]);
                                        $sms->sendSms();
                                        break;
                                    case 'email':
                                        $c_template = unserialize($col['c_template']);
...
//            $modelsRelated->setTableName('stock');
            ...
            echo $this->renderPartial('@app/views/crud/related-dynamic', [

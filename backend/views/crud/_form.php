...
?>
    ...
    if($model->isNewRecord){
        if( Yii::$app->hasEventHandlers('backend/crud/form/submit/add') ){
            ...
            echo Html::submitButton( Icon::show('plus').' '.Yii::t('admin', 'Create'), ['class' => 'btn btn-success']);
        }
    }else{
        if(Yii::$app->hasEventHandlers('backend/crud/form/submit/update') ){
            Yii::$app->trigger('backend/crud/form/submit/update');
        }else{
            echo Html::submitButton( Icon::show('save').' '.Yii::t('admin', 'Update'), ['class' => 'btn btn-primary']);
        }
    }
    ...
    BackendWidget::begin(
        [
            'icon' => 'user',
            'title' => $model->isNewRecord ?Yii::t('admin', 'Create'):Yii::t('admin', 'Update'),
            //'showLabels' => true,
            'footer' => $footer,
        ]
    );
        ...
        foreach($fields as $k => $v){
            ...
            if(true === $workflow){
                    ...
                    switch ($v->attributes['field_type']) {
                        ...
                        case 'select':
                            if (!empty($v->attributes['sql_query'])) {
                                ...
                                if(sizeof($results) > 0){
                                    $c[''] = Yii::t('admin' , '-- Select --' );
                                    foreach($results as $result){
                                        list($a, $b) = array_values($result);
                                        $c[$a] = $b;
                                    }
                                    echo $field->dropDownList($c,['disabled' => 'disabled']);
                                }
                ...
                if($v->attributes['name'] == 'status'){
                    ...
                    if(count($values) == 0){
                        echo $field->dropDownList([' '=>\Yii::t('admin','You do not have permission to change status')],['readonly' => true]);
                        continue;
                    }
                    ...
                    case 'multi_select':
                        if (!empty($v->attributes['custom_func'])) {
                            ...
                            if(sizeof($results) > 0){
                                ...
                                $field->widget(\kartik\widgets\Select2::classname(), [
                                    'data' => $c,
                            ...
                            $field->dropDownList($values,['multiple' => true]);
                        }
                        break;
                    case 'select':
                        if (!empty($v->attributes['custom_func'])) {
...
//                            }
//                            print_r($field);die();
                            $field->checkboxList($values);
                        }
//                    {if $af.additional_fields.custom_func != ''}
//            {php}
//            $this->assign('custom_func' , eval($this->_tpl_vars['af']['additional_fields']['custom_func']));
//            {/php}
//            {html_options options=$custom_func selected=$f[$af.name]|default:$af.additional_fields.default}
//            {elseif $af.additional_fields.sql_query != ''}
//            {php}
//            unset($c);
//            $this->assign('sql_query' , '');
//            $d = $GLOBALS['db']->getAll($this->_tpl_vars['af']['additional_fields']['sql_query']);
//            if(is_array($d) && count($d) > 0){
//                foreach($d as $v){
//                    list($a, $b) = array_values($v);
//                    $c[$a] = $b;
//                }
//                $this->assign('sql_query' , $c);
//            }
//            {/php}
//            {html_checkboxes name="additional_fields[`$af.name`]" options=$sql_query selected=$f[$af.name]|unserialize|default:$af.additional_fields.default separator='<br />'}
//            {else}
//            {html_checkboxes name="additional_fields[`$af.name`]" options=$af.additional_fields.options selected=$f[$af.name]|unserialize|default:$af.additional_fields.default separator='<br />'}
//            {/if}
...
if(!$model->isNewRecord) {
    ?><div class="row">
        <div class="col-md-12">
            <div class="panel panel-default grid-view"><div class="panel-body"> <?php
    echo Comments\widgets\CommentListWidget::widget([
        'entity' => (string)$this->context->table_name . '-' . $model->id, // type and id
    ]);
    ?></div> </div></div></div> <?php
}

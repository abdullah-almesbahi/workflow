<?php
namespace backend\modules\mailer\components;
...
class BaseMailer extends Component
{
     ...
     * @param array $template_partials TML code to be inserted in the email. array('__key_for_partial__' => '<h1>html</h1>').
     *        Defaults to array();
	...
	public static function compileMultipartTemplate($template, $template_partials = array()) {
		...
		if (!is_a($template, 'backend\modules\mailer\models\MailerTemplate')) {
			throw new \yii\db\Exception('Mailer::compileMultipartTemplate(): '. Yii::t(
				'admin',
	...
	private static function process($to, $template_name, $template_vars = array(), $template_partials = array(), $action = 'send') {
		...
		$template = MailerTemplate::find()->where(['name' => $template_name])->one();
		if (!$template) {

			throw new \yii\db\Exception(Yii::t('admin', 'Template does not exists.'));
		}

		//Template compilation
		$compiled_template = self::compileMultipartTemplate($template, $template_partials);
//        print_r($compiled_template);die();
		...
		foreach ($to as $receiver) {
			...
			if ($template_vars) {
                ...
                $template->subject = strtr($template->subject, $template_vars[$i]);
            }else
		...
		foreach ($statuses as $email => $status) {
			if (!$status) {
				Yii::log(Yii::t('admin', 'Failed delivering some emails.'), 'error');
				return false;
			}
		}
		return true;
	...
	public static function sendToList($list_name, $template_name, $template_partials = array()) {
		$db = Yii::$app->db;
		$list = MailerList::find()->where(['name' => $list_name])->all();
		if (!$list)
			throw new \yii\db\Exception('Mailer::sendToList(): ' . Yii::t('admin', "List doesn't exists."));
		...
		try {
			...
			throw new \yii\db\Exception("Mailer::sendToList(): " . $e->getMessage());
		}
	}
}

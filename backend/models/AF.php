...
class AF extends ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['name'],'match', 'pattern' => '/^[a-z0-9_]+$/' , 'enableClientValidation' => false , 'message' => Yii::t('admin','Name must be entered and it may contain lowercase letters, underscopes and digits')],
            // [['title'],'match', 'pattern' => '/.+/' , 'enableClientValidation' => false , 'message' => Yii::t('admin','Name must be entered and it may contain lowercase letters, underscopes and digits')],
            ['name' , 'in' , 'not' =>  true  , 'range' => ['id','sort','create_time','update_time'] , 'message' => Yii::t('admin','Please choose another field name. This is already used') ],
        ];
    }
...
//        }
    ...
    public function isWorkflowEnabled(){
        $status = $this->getStatusField();
        return (isset($status['enable_workflow']) && 1 == $status['enable_workflow'])?true:false;
    }

...
class WorkflowPhpSource extends Object implements IWorkflowSource
{
	...
	public function getWorkflowDefinition($id)
	{
		...
		if ( ! isset($this->_workflowDef[$id]) ) {
			...
			if ( $this->isWorkflowProvider($wfProvider , $id)) {
				if(strpos($id, '1')) {
					$this->_workflowDef[$id] = $this->parse($id, $wfProvider->getDefinition());
				}elseif(strpos($id, '2')){
					$this->_workflowDef[$id] = $this->parse($id, $wfProvider->getDefinition2());
				}else{
					$this->_workflowDef[$id] = $this->parse($id, $wfProvider->getDefinition());
				}
			} else {
	 ...
	 * @return boolean
	 */
	public function isWorkflowProvider($object , $id = null)
	{
		if(!is_null($id)){
			if(strpos($id, '1')) {
				return method_exists($object, 'getDefinition');
			}elseif(strpos($id, '2')){
				return method_exists($object, 'getDefinition2');
			}else{
				return method_exists($object, 'getDefinition');
			}
		}

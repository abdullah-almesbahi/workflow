...
class ImageDropzone extends DropZone
{
    ...
    public static function saveThumbnail($dir, $filename, $width=80, $height=80)
    {
        ...
        if (trim($filename) && file_exists(Yii::getAlias($dir . '/' . $filename)) && substr($filename,-3) != 'rar' && substr($filename,-3) != 'zip' ) {
    ...
    protected function addFiles($files = [])
    {
        ...
        $URLpath = Yii::getAlias('@web' . $this->uploadDir . '/');
        ...
        foreach ($files as $file) {
            ...
            if(substr($file['name'],-3) == 'rar' || substr($file['name'],-3) == 'zip' ){
                $jquery = 'jQuery(' . $fhName . '.previewElement).find(".preview a").attr("href" , "'.$URLpath.$file['name'].'");
                jQuery(' . $fhName . '.previewElement).find(".preview a").removeAttr("data-toggle");
                ';
            }else{
                $jquery = 'jQuery(' . $fhName . '.previewElement).find(".preview a").attr("href" , "'.$URLpath.$file['name'].'");';
            }
    ...
    public function init()
    {
        ...
        /** Image $file */
        foreach ($files as $file) {
            if (!isset($file->thumbnail_src) || !trim($file->thumbnail_src) && (substr($file->filename,0))) {
        ...
        $this->eventHandlers = [
            ...
            'success' => 'function(file, response) {
                ...
                jQuery(file.previewElement).find(".preview a").attr("href" , "'.Yii::getAlias('@web').'"+response.afterUpload.image_src);
